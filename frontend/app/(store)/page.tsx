      ...
      <section className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white py-20 px-4">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-10">
          ...
          <div className="hidden md:flex gap-4">
            {[
              { icon: "M12 2C8 2 6 5 6 8c0 2 1 3.5 2.5 4.5L10 21h4l1.5-8.5C17 11.5 18 10 18 8c0-3-2-6-6-6zM9 8c0-1.7 1.3-3 3-3s3 1.3 3 3-1.3 3-3 3-3-1.3-3-3z", label: "Pipes & Fittings", slug: "pipes-fittings" },
              { icon: "M13 10V3L4 14h7v7l9-11h-7z", label: "Electrical", slug: "electrical" },
              { icon: "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6", label: "Sanitary Ware", slug: "sanitary-ware" },
            ].map((item) => (
              <Link key={item.label} href={`/products?category=${item.slug}`} className="w-28 h-28 bg-slate-800 border border-slate-700 rounded-2xl flex flex-col items-center justify-center gap-2 hover:border-orange-500 hover:bg-slate-700 transition-all duration-200 cursor-pointer">
                <svg className="w-8 h-8 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={item.icon} />
                </svg>
                <span className="text-xs text-slate-300 font-medium text-center px-2">{item.label}</span>
              </Link>
            ))}
          </div>
        </div>
